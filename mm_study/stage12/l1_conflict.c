...
#define CONFLICT_STRIDE		PAGE_SIZE
#define CONTROL_STRIDE		CACHE_LINE
...
/*
 * Generate one deterministic random permutation.
 *
 * The same order is used for control and conflict, so only the address
 * placement changes; the logical pointer-chase sequence is identical.
 */
...
int main(void)
{
	...
	printf("L1D                  : 32 KiB, 64 sets, 8-way\n");
	printf("cache line           : 64 B\n");
	printf("L1 set index         : page-offset bits [11:6]\n");
	printf("control stride       : 64 B\n");
	printf("conflict stride      : 4096 B\n");
	...
	for (i = 0; i < ARRAY_SIZE(line_counts); i++) {
		...
		conflict_start = build_chain(conflict_base, CONFLICT_STRIDE,
					     order, nlines);
